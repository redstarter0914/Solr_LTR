...
class MyLog:
    log_filename = os.curdir+"/logs/py_log.log"
    err_filename = os.curdir+'/logs/py_err.log'
    true_filename = os.curdir+'/logs/py_true.log'
    dateformat = '%Y-%m-%d %H:%M:%S'
    logger.setLevel(LEVELS.get(level, logging.NOTSET))
    createFile(log_filename)
    createFile(err_filename)
    createFile(true_filename)

#注意文件内容写入时编码格式指定

    handler = logging.FileHandler(log_filename, encoding='utf-8')
    errhandler = logging.FileHandler(err_filename, encoding='utf-8')
    truehandler = logging.FileHandler(true_filename, encoding='utf-8')
    ...
    def error(log_message):
        setHandler('error')
        errstr = str("[ERROR "+getCurrentTime()+"]"+log_message)
        logger.error(errstr)
...
def setHandler(level):
    ...
    #把logger添加上handler
        ...
        logger.addHandler(MyLog.truehandler)
    else:
...
def removerhandler(level):
    if level == 'error':
        ...
        logger.removeHandler(MyLog.truehandler)
    else:
